...
for cells in cell_configs:
    ...
    # Initialize the solver with the current configuration
    solver = GreedyBestFirstSearch(cells, 3)  # Assuming solver is BestFirstSearch, not GBFS
    duration, num_steps, path = solver.findMinimumSteps()
    
    if num_steps == -1:
        print("No solution found.")
    else:
        print("Move Sequence:", " -> ".join(path))  # Joining the moves to make it a readable sequence
        print("Total Steps:", num_steps)
        print("Solving Time (seconds):", duration)  # Duration for solving
        print("Final State:", path[-1])  # Assuming path[-1] is the final state
